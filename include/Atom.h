...
struct Atom
{
    ...
    /// dipole derivatives calculated by neural network.
    Vec3D                    dDipole[3];
    /// Reference dipole derivtives from data set.
    Vec3D                    dDipoleRef[3];
#endif
     ...
     * @param[in] convDipole Multiplicative dipole unit conversion factor.
     */
#ifdef DIPOLE_DERIVATIVES
    ...
    /** Update force RMSE with dipole derivatives of this atom.
     *
     * @param[in,out] rmse Input RMSE to be updated.
     * @param[in,out] count Input counter to be updated.
     */
...
#ifdef DIPOLE_DERIVATIVES
    void                     updateRmseDDipole(double&      rmse,
                                              std::size_t& count) const;
    /** Get reference and NN forces for this atoms.
     *
     * @return Vector of strings with #indexStructure, #index, #dDipoleRef, #dDipole
     * values.
     */
    std::vector<std::string> getDDipoleLines() const;
    /** Get atom information as a vector of strings.
     *
     * @return Lines with atom information.
     */
