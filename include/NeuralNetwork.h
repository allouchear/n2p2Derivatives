...
public:
    ...
    /** Return number of neurons for a given layer.
     *
     */
    int                      getNumNeurons(int numLayer) const;
    ...
    /** Calculate decond derivatives of output neuron with respect to input neurons.
     *
     * @param[out] d2EdG2 Array 2D containing derivatives (length is number of input
     *                  neurons*length is number of input neurons).
     *
     * __CAUTION__: This works only for neural networks with a single output neuron!
     *
     * Returns @f$\left(\frac{d2E}{dG_idG_j}\right)_{i=1,\ldots,N, j=1,\ldots,N}@f$, where
     * @f$E@f$ is the output neuron and @f$\left(G_i\right)_{i=1,\ldots,N}@f$
     * are the @f$N@f$ input neurons.
     */
    void calculatedEdG(double **za, int ia) const;
    void calculated2EdG(double **zab, double**za,  double**zb) const;
    void calculated3EdG(double** zabc, double **zab,  double **zac, double **zbc, double**za,  double** zb, double** zc) const;
    void calculated4EdG(double** zabcd, 
    ...
    void calculatedEdG(double *dEdG) const;
    void calculateD2EdG2(double **d2EdG2) const;
    void calculateD3EdG3(double ***d3EdG3) const;
    void calculateD4EdG4(double ****d4EdG4) const;
    void calculateDnEdGn(double* dEdG, double** d2EdG2, double*** d3EdG3, double ****d4EdG4) const;
#endif
...
private:
    ...
    typedef struct
    {
        ...
        double  d2fdx2;
#ifdef HIGH_DERIVATIVES
        /// Third derivative of activation function with respect to its
        /// argument \f$ \frac{\partial^2 f_a}{\partial x^2} \f$.
        double  d3fdx3;
        /// Forth derivative of activation function with respect to its
        /// argument \f$ \frac{\partial^2 f_a}{\partial x^2} \f$.
        double  d4fdx4;
#endif
...
#ifdef HIGH_DERIVATIVES
    double** newztable() const ;
    void freeztable(double**z) const;
#endif
};
