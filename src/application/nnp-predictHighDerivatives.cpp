...
double**** getNumericFourth(Prediction& prediction,double step)
{
	...
	*/
	...
	for(int c=0;c<3;c++)
	{
		...
		auto gp = prediction.getHighDerivatives(3);

    		prediction.readStructureFromFile("input.data");
		prediction.structure.atoms[i].r[c] -= step;
		auto gm = prediction.getHighDerivatives(3);
...
double*** getNumericThird(Prediction& prediction,double step)
{
	int nAtoms = prediction.structure.numAtoms;
	int nAtoms3 = 3*nAtoms;
	double step2=1.0/step/2.0;
...
int main(int argc, char* argv[])
{
    ...
    timing(cpua,sysa);
    auto deriv2 = prediction.getHighDerivatives(2);
    timing(cpua2,sysa2);
...
/////////////////////////////////// Third derivatives
    ...
    timing(tmpc,tmps);
    auto d3fNumeric = getNumericThird(prediction, step);
    timing(cpun3,sysn3); cpun3 -= tmpc; sysn3 -= sysn3;

    prediction.log << " Begin analytical third derivatives\n";
    double cpua3, sysa3;
    timing(tmpc,tmps);
    auto deriv3 = prediction.getHighDerivatives(3,0);
...
/////////////////////////////////// Fourth derivatives
    ...
    timing(tmpc,tmps);
    auto d4fNumeric = getNumericFourth(prediction, step);
    timing(cpun4,sysn4); cpun4 -= tmpc; sysn4 -= tmps;

    prediction.log << " Begin analytical fourth derivatives\n";
    double cpua4, sysa4;
    timing(tmpc,tmps);
    auto deriv4 = prediction.getHighDerivatives(4,0);
