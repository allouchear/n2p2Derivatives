...
public:
    ...
    /** Derivative of cutoff function @f$\frac{d^n f_c}{d r^n}@f$.
     *
     * @param[in] r Distance.
     * @param[in] order  integer ) order of derivative (1, 2, 3, 4, ...).
     * @return Value of cutoff function derivative.
     */
     double dnf(double r, int order) const;
#ifdef HIGH_DERIVATIVES
    double getFirstDerivatives(const double* rij, double firstDeriv[]) const;
    double get12Derivatives(const double* rij,  double firstDeriv[], double secondDeriv[][6]) const;
#endif
...
private:
    ...
    void       (CutoffFunction::*fdfPtr)(double  r,
                                         ...
                                         double& dfc) const;
#ifdef HIGH_DERIVATIVES
    /// Function pointer to dnf.
    double     (CutoffFunction::*dnfPtr)(double r, int order) const;
#endif
...
#ifdef HIGH_DERIVATIVES
    double  dnfHARD (double r, int order) const;
#endif
...
#ifdef HIGH_DERIVATIVES
    double  dnfCOS (double r, int order) const;
#endif
...
#ifdef HIGH_DERIVATIVES
    double  dnfTANHU (double r, int order) const;
#endif
...
#ifdef HIGH_DERIVATIVES
    double  dnfTANH (double r, int order) const;
#endif
...
#ifdef HIGH_DERIVATIVES
    double  dnfPOLY1 (double r, int order) const;
#endif
...
#ifdef HIGH_DERIVATIVES
    double  dnfPOLY2 (double r, int order) const;
#endif
...
#ifdef HIGH_DERIVATIVES
    double  dnfPOLY3 (double r, int order) const;
#endif
...
#ifdef HIGH_DERIVATIVES
    double  dnfPOLY4 (double r, int order) const;
#endif
...
#ifdef HIGH_DERIVATIVES
    double  dnfEXP (double r, int order) const;
#endif
};
...
inline double CutoffFunction::dnf(double r, int order) const
{
    if (r >= rc) return 0.0;
    return (this->*dnfPtr)(r,order);
}
...
#ifdef HIGH_DERIVATIVES
inline double CutoffFunction::dnfHARD(double /*r*/, int order) const
{
