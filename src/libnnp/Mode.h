 ...
 * file.close();
 * ```
 * we can finally predict the energy, forces, charge and dipole from the neural network
 * potential:
 * ```
 ...
 * mode.calculateEnergyAndForces(structure);
 * mode.calculateCharge(structure);
...
public:
    ...
    /** Set up dftd3.
     *
     * If the keywords `dftd3_func`is present, the provided functional
     * is used to set parameters of DFTD3 
     */
    void                     setupDFTD3();
     ...
     *
     * @param[in] structure Input structure.
     * The atomic charge is stored in Atom::charge. 
     */
    void                     calculateAtomicChargesNeuralNetworks( Structure& structure, bool const derivatives) const;
    ...
    /** Calculate ENergy & forces for all atoms in given structure.
     *
     * @param[in] structure Input structure.
     *
     * Sum up potential energy from atomic energy contributions. Result is
     ...
     * computation to atomic forces. Results are stored in Atom::f.
     */
    void 	     	    calculateEnergyAndForces(Structure& structure) const;
    /** Calculate charge for a given structure.
     *
     * @param[in] structure Input structure.
     *
    ...
    /** Calculate dipole for a given structure.
     *
     * @param[in] structure Input structure.
     *
     * Sum up dipole from atomic charge contributions. Result is
     * stored in Structure::dipole.
     */
    void                     calculateDipole(Structure& structure) const;
    /** Calculate first derivative of dipole for a given structure.
     *
     * @param[in] structure Input structure.
     *
     * @param[out] derivatives : [3][3*nAtoms]
     */
    double** 		     computedDipole(Structure& structure) const;
#ifdef HIGH_DERIVATIVES
    /** Calculate seconde derivatives for a given structure.
     *
     * @param[in] structure Input structure.
     *
     * return 3nAtoms*(3nAtoms+1)/2 vector:  matrix inf
     * return derivSize = 3nAtoms*(3nAtoms+1)/2 
     */
    double *computeHessian(Structure& structure, size_t & derivSize) const;
    double** computed2Dipole(Structure& structure, size_t& derivSize) const;
    Derivatives* computeHighDerivativesDipole(Structure& structure, const int order, int method) const;
    Derivatives* computeHighDerivativesDipoleMem(Structure& structure, const int order) const;
    Derivatives* computeHighDerivativesDipoleFile(Structure& structure, const int order) const;
    Derivatives* computeHighDerivativesDipole(Structure& structure, const int order) const;
    Derivatives computeHighDerivatives(Structure& structure, const int order, int method) const;
    Derivatives computeHighDerivativesFile(Structure& structure, const int order) const;
    Derivatives computeHighDerivativesMem(Structure& structure, const int order) const;
    Derivatives computeHighDerivatives(Structure& structure, const int order) const;
    Derivatives computeFourthFile(Structure& structure) const;
    Derivatives computeFourthMem(Structure& structure) const;
    Derivatives computeFourth(Structure& structure) const;
    Derivatives computeThirdFile(Structure& structure) const;
    Derivatives computeThirdMem(Structure& structure) const;
    Derivatives computeThird(Structure& structure) const;
    Derivatives computeHessian(Structure& structure) const;
#endif
     ...
     */
    double                   normalizedDipole(double dipole) const;
    /** Apply normalization to given dipole of structure.
     *
     * @param[in] structure Input structure with dipole in physical units.
     * @param[in] ref If true, use reference dipole, otherwise use NN dipole.
     *
     * @return Dipole in normalized units.
     */
    Vec3D                   normalizedDipole(
                                            Structure const& structure,
                                            bool             ref = true) const;
    /** Apply normalization to given ddipole.
     *
     * @param[in] ddipole Input ddipole in physical units.
     *
     * @return DDipole in normalized units.
     */
    double                   normalizedDDipole(double ddipole) const;
    /** Undo normalization for a given dipole.
     *
     * @param[in] dipole Input dipole in normalized units.
     *
     * @return Dipole in physical units.
     */
    double                   physicalDipole(double dipole) const;
    /** Undo normalization for a given dipole of structure.
     *
     * @param[in] structure Input structure with dipole in normalized units.
     * @param[in] ref If true, use reference dipole, otherwise use NN dipole.
     *
     * @return Dipole in physical units.
     */
    Vec3D                   physicalDipole(Structure const& structure,
                                            bool             ref = true) const;
    /** Undo normalization for a given ddipole.
     *
     * @param[in] ddipole Input ddipole in normalized units.
     *
     * @return DDipole in physical units.
     */
    double                   physicalDDipole(double ddipole) const;
    /** Convert one structure to normalized units.
     *
     * @param[in,out] structure Input structure.
     */
    ...
    /** Getter for Mode::convDipole.
     *
     * @return diopole unit conversion factor.
     */
    double                   getConvDipole() const;
...
inline double Mode::getConvDipole() const
{
    return convDipole;
}
