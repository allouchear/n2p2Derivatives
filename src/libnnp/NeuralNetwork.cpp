...
int NeuralNetwork::getNumNeurons(int numLayer) const
{
	return layers[numLayer].numNeurons;
}
...
V.I. Avrutskiy
https://arxiv.org/abs/1712.04185
Allouche : Tested and validated
*/
/* za[l-1] = input for layer number l 
   except for z[numLayers-1] : it is the output for layer number numLayer-1
*/
void NeuralNetwork::calculatedEdG(double **za, int ia) const
{
	int l = 1;
	int lm1 = l-1;
        for (int i = 0; i < layers[l].numNeurons; i++) za[lm1][i] = layers[l].neurons[i].weights[ia];

        for (int l = 2; l < numLayers; l++)
        {
	    int lm1 = l-1;
	    int lm2 = l-2;
            for (int il = 0; il < layers[l].numNeurons; il++)
            {
                za[lm1][il] = 0.0;
                for (int ilm1 = 0; ilm1 < layers[lm1].numNeurons; ilm1++)
                {
                    za[lm1][il] += layers[l].neurons[il].weights[ilm1]*za[lm2][ilm1]*layers[lm1].neurons[ilm1].dfdx;
                }
                if (normalizeNeurons && l>1) za[lm1][il] /= layers[lm1].numNeurons;
            }
        }
	...
	lm1 = l-1;
        for (int il = 0; il < layers[l].numNeurons; il++)
            {
                za[l][il] = za[lm1][il]*layers[l].neurons[il].dfdx;
	    }
}
/* based on :
Backpropagation generalized for output derivatives
...
Allouche : Tested and validated
*/
/* z[l-1] = input for layer number l 
   except for z[numLayers-1] : it is the output for layer number numLayer-1
*/
void NeuralNetwork::calculated2EdG(double **zab, double**za,  double** zb) const
{
	int l = 1;
	int lm1 = l-1;
        for (int i = 0; i < layers[l].numNeurons; i++) zab[lm1][i] = 0;
	    ...
	    int lm2 = l-2;
            for (int il = 0; il < layers[l].numNeurons; il++)
            {
                ...
                for (int ilm1 = 0; ilm1 < layers[lm1].numNeurons; ilm1++)
                {
			zab[lm1][il] += layers[l].neurons[il].weights[ilm1]*(
				  zab[lm2][ilm1]*layers[lm1].neurons[ilm1].dfdx
				+ za[lm2][ilm1]*zb[lm2][ilm1]*layers[lm1].neurons[ilm1].d2fdx2);
                }
                if (normalizeNeurons && l>1) zab[lm1][il] /= layers[lm1].numNeurons;
            }
        }
	// output of last layer
	l = numLayers-1;
	lm1 = l-1;
        for (int il = 0; il < layers[l].numNeurons; il++)
            {
		zab[l][il] = ( zab[lm1][il]*layers[l].neurons[il].dfdx
				+ za[lm1][il]*zb[lm1][il]*layers[l].neurons[il].d2fdx2);
	    }
}
/* Based on :
Enhancing approximation abilities of neural networks by training derivatives
V.I. Avrutskiy
https://arxiv.org/abs/1712.04473
Allouche : calculated3EdG Ok tested
*/
void NeuralNetwork::calculated3EdG(double** zabc, 
			double **zab,  double **zac, double **zbc,
			double** za,  double** zb, double** zc) const
{
	int l = 1;
	int lm1 = l-1;
        for (int i = 0; i < layers[l].numNeurons; i++) zabc[lm1][i] = 0;
	    ...
	    int lm2 = l-2;
            for (int il = 0; il < layers[l].numNeurons; il++)
            {
                ...
                for (int ilm1 = 0; ilm1 < layers[lm1].numNeurons; ilm1++)
                {
			zabc[lm1][il] += layers[l].neurons[il].weights[ilm1]*
				(
				    (    zabc[lm2][ilm1]
				    )*layers[lm1].neurons[ilm1].dfdx
				  + (	zab[lm2][ilm1]*zc[lm2][ilm1]+
					zac[lm2][ilm1]*zb[lm2][ilm1]+
					zbc[lm2][ilm1]*za[lm2][ilm1]
				   )*layers[lm1].neurons[ilm1].d2fdx2
				  + (
					za[lm2][ilm1]*zb[lm2][ilm1]*zc[lm2][ilm1]
				    )*layers[lm1].neurons[ilm1].d3fdx3
				);
                }
                if (normalizeNeurons && l>1) zabc[lm1][il] /= layers[lm1].numNeurons;
            }
        }
	// output of last layer
	l = numLayers-1;
	lm1 = l-1;
        for (int il = 0; il < layers[l].numNeurons; il++)
            {
		zabc[l][il] =       (    zabc[lm1][il]
				    )*layers[l].neurons[il].dfdx
				  + (	zab[lm1][il]*zc[lm1][il]+
					zac[lm1][il]*zb[lm1][il]+
					zbc[lm1][il]*za[lm1][il]
				   )*layers[l].neurons[il].d2fdx2
				  + (
					za[lm1][il]*zb[lm1][il]*zc[lm1][il]
				    )*layers[l].neurons[il].d3fdx3;
	    }
}
/* Based on :
Enhancing approximation abilities of neural networks by training derivatives
...
https://arxiv.org/abs/1712.04473
*/
void NeuralNetwork::calculated4EdG(double** zabcd, 
			double **zabc,  double **zabd,  double **zacd,  double** zbcd,
			double **zab,  double **zac, double** zad, double **zbc,  double **zbd, double** zcd, 
			double**za,  double** zb, double** zc, double** zd) const
{
	int l = 1;
	int lm1 = l-1;
        for (int i = 0; i < layers[l].numNeurons; i++) zabcd[lm1][i] = 0;
	    ...
	    int lm2 = l-2;
            for (int il = 0; il < layers[l].numNeurons; il++)
            {
                ...
                for (int ilm1 = 0; ilm1 < layers[lm1].numNeurons; ilm1++)
                {
			zabcd[lm1][il] += layers[l].neurons[il].weights[ilm1]*(
				     (  zabcd[lm2][ilm1]
				     )*layers[lm1].neurons[ilm1].dfdx
				  + (	zab[lm2][ilm1]*zcd[lm2][ilm1]+
				        zac[lm2][ilm1]*zbd[lm2][ilm1]+	
				        zad[lm2][ilm1]*zbc[lm2][ilm1]+	
				        za[lm2][ilm1]*zbcd[lm2][ilm1]+	
				        zb[lm2][ilm1]*zacd[lm2][ilm1]+	
				        zc[lm2][ilm1]*zabd[lm2][ilm1]+	
				        zd[lm2][ilm1]*zabc[lm2][ilm1]
				   )*layers[lm1].neurons[ilm1].d2fdx2
				  + (
					zab[lm2][ilm1]*zc[lm2][ilm1]*zd[lm2][ilm1]+
					zac[lm2][ilm1]*zb[lm2][ilm1]*zd[lm2][ilm1]+
					zad[lm2][ilm1]*zb[lm2][ilm1]*zc[lm2][ilm1]+
					zbc[lm2][ilm1]*za[lm2][ilm1]*zd[lm2][ilm1]+
					zbd[lm2][ilm1]*za[lm2][ilm1]*zc[lm2][ilm1]+
					zcd[lm2][ilm1]*za[lm2][ilm1]*zb[lm2][ilm1]
				    )*layers[lm1].neurons[ilm1].d3fdx3
				  + (
					za[lm2][ilm1]*zb[lm2][ilm1]*zc[lm2][ilm1]*zd[lm2][ilm1]
				    )*layers[lm1].neurons[ilm1].d4fdx4
				);
                }
                if (normalizeNeurons && l>1) zabc[lm1][il] /= layers[lm1].numNeurons;
            }
	...
	lm1 = l-1;
		...
		zabcd[l][il] = (
				  ...
				  + (	zab[lm1][il]*zcd[lm1][il]+
				        zac[lm1][il]*zbd[lm1][il]+	
				        zad[lm1][il]*zbc[lm1][il]+	
				        za[lm1][il]*zbcd[lm1][il]+	
				        zb[lm1][il]*zacd[lm1][il]+	
				        zc[lm1][il]*zabd[lm1][il]+	
				        zd[lm1][il]*zabc[lm1][il]
				   )*layers[l].neurons[il].d2fdx2
				  + (
					zab[lm1][il]*zc[lm1][il]*zd[lm1][il]+
					zac[lm1][il]*zb[lm1][il]*zd[lm1][il]+
					zad[lm1][il]*zb[lm1][il]*zc[lm1][il]+
					zbc[lm1][il]*za[lm1][il]*zd[lm1][il]+
					zbd[lm1][il]*za[lm1][il]*zc[lm1][il]+
					zcd[lm1][il]*za[lm1][il]*zb[lm1][il]
				    )*layers[l].neurons[il].d3fdx3
				  + (
					za[lm1][il]*zb[lm1][il]*zc[lm1][il]*zd[lm1][il]
				    )*layers[l].neurons[il].d4fdx4
				);
	    }
}
double** NeuralNetwork::newztable() const
{
    double** z = new double*[numLayers];
    for (int l = 0; l < numLayers-1; l++) z[l] = new double[layers[l+1].numNeurons];
    z[numLayers-1] = new double[layers[numLayers-1].numNeurons];
    return z;
}
void NeuralNetwork::freeztable(double**z) const
{
    for (int l = 0; l < numLayers; l++) delete[] z[l];
    delete[] z;
}
void NeuralNetwork::calculatedEdG(double *dEdG) const
{
    ...
    {
		calculatedEdG(za, k);
        	dEdG[k] = za[numLayers-1][0];
    }

    freeztable(za);
}
void NeuralNetwork::calculateD2EdG2(double **d2EdG2) const
{
    double** za = newztable();
    double** zb = newztable();
    double** zab = newztable();

// matrix inf
    for (int ka = 0; ka < layers[0].numNeurons; ka++)
    {
	calculatedEdG(za, ka);
	calculated2EdG(zab, za, za);
        d2EdG2[ka][ka] = zab[numLayers-1][0];
    	for (int kb = 0; kb <=ka; kb++)
	{
		calculatedEdG(zb, kb);
		calculated2EdG(zab, za, zb);
        	d2EdG2[ka][kb] = zab[numLayers-1][0];
	}
    }
...
void NeuralNetwork::calculateD3EdG3(double ***d3EdG3) const
{
    double** za = newztable();
    double** zb = newztable();
    double** zc = newztable();
    double** zab = newztable();
    double** zac = newztable();
    double** zbc = newztable();
    double** zabc = newztable();
    int nG = layers[0].numNeurons;
    	...
    	for (int kb = 0; kb <=ka; kb++)
	{
    		...
    		for (int kc = 0; kc <=kb; kc++)
		{
			...
			calculated2EdG(zac, za, zc);
			calculated2EdG(zbc, zb, zc);

			calculated3EdG(zabc, zab, zac, zbc, za, zb, zc);
			int arr[3] = {ka,kb,kc};
			d3EdG3[ka][kb][kc] = zabc[numLayers-1][0];
		}
	}
    }
...
void NeuralNetwork::calculateD4EdG4(double ****d4EdG4) const
{
    double** za = newztable();
    double** zb = newztable();
    double** zc = newztable();
    ...
    double** zab = newztable();
    double** zac = newztable();
    double** zad = newztable();
    double** zbc = newztable();
    double** zbd = newztable();
    double** zcd = newztable();

    double** zabc = newztable();
    double** zabd = newztable();
    double** zacd = newztable();
    double** zbcd = newztable();

    double** zabcd = newztable();
    	...
    	for (int kb = 0; kb <=ka; kb++)
	{
    		...
    		for (int kc = 0; kc <=kb; kc++)
		{
    			...
    			for (int kd = 0; kd <= kc; kd++)
			{
				...
				calculated2EdG(zad, za, zd);
				calculated2EdG(zbd, zb, zd);
				calculated2EdG(zcd, zc, zd);

				calculated3EdG(zabd, zab, zad, zbd, za, zb, zd);
				calculated3EdG(zacd, zac, zad, zcd, za, zc, zd);
				calculated3EdG(zbcd, zbc, zbd, zcd, zb, zc, zd);

				calculated4EdG(zabcd, zabc,  zabd,  zacd,  zbcd, zab,  zac, zad, zbc,  zbd, zcd, za,  zb, zc, zd);
				int arr[4] = {ka,kb,kc,kd};
				d4EdG4[ka][kb][kc][kd] = zabcd[numLayers-1][0];
			}
		}
	}
    }
...
void NeuralNetwork::calculateDnEdGn(double* dEdG, double** d2EdG2, double*** d3EdG3, double ****d4EdG4) const
{
    double** za = NULL;
    double** zb = NULL;
    double** zc = NULL;
    ...
    if(zc) freeztable(zc);
    if(zd) freeztable(zd);
    if(zab) freeztable(zab);
    if(zac) freeztable(zac);
    if(zad) freeztable(zad);
    if(zbc) freeztable(zbc);
    if(zbd) freeztable(zbd);
    if(zcd) freeztable(zcd);
    if(zabc) freeztable(zabc);
    if(zabd) freeztable(zabd);
    if(zacd) freeztable(zacd);
    if(zbcd) freeztable(zbcd);
    if(zabcd) freeztable(zabcd);
}
#endif
...
#ifdef HIGH_DERIVATIVES
        layer.neurons[i].d3fdx3 = 0.0;
        layer.neurons[i].d4fdx4 = 0.0;
#endif
    ...
    for (int i = 0; i < layer.numNeurons; i++)
    {
...
#ifdef HIGH_DERIVATIVES
	    double dtmp2 = dtmp*dtmp;
	    double dtmp4 = dtmp2*dtmp2;
            layer.neurons[i].d3fdx3 = -2-6*dtmp4 + 8*dtmp2;
            layer.neurons[i].d4fdx4 = (24*dtmp4-40*dtmp2+16)*dtmp;
#endif 
        }
        else if (layer.activationFunction == AF_LOGISTIC)
        {
            ...
            else
            {
                ...
                layer.neurons[i].d2fdx2 = dtmp * (1.0 - dtmp)
                                        * (1.0 - 2.0 * dtmp);
#ifdef HIGH_DERIVATIVES
            	layer.neurons[i].d3fdx3 = dtmp*(1+dtmp*(-7+dtmp*(12-6*dtmp)));
            	layer.neurons[i].d4fdx4 = dtmp*(1+dtmp*(-15+dtmp*(50+dtmp*(-60+24*dtmp))));
#endif 
            }
        }
        else if (layer.activationFunction == AF_SOFTPLUS)
        {
            ...
            else
            {
                ...
                layer.neurons[i].d2fdx2 = dtmp * (1.0 - dtmp);
#ifdef HIGH_DERIVATIVES
            	layer.neurons[i].d3fdx3 = dtmp*(1+dtmp*(2*dtmp-3));
            	layer.neurons[i].d4fdx4 = dtmp*(1+dtmp*(-7+dtmp*(12-6*dtmp)));
#endif 
            }
        }
...
#ifdef HIGH_DERIVATIVES
	    double dtmp2 = dtmp*dtmp;
            layer.neurons[i].d3fdx3 = -dtmp*tmpexp*(dtmp2-3);
            layer.neurons[i].d4fdx4 = tmpexp*(dtmp2*dtmp2-6*dtmp2+3);
#endif 
        }
        else if (layer.activationFunction == AF_COS)
        {
            double const tmpcos = cos(dtmp);
            double const tmpsin = sin(dtmp);
            layer.neurons[i].value  = tmpcos;
            layer.neurons[i].dfdx   = -tmpsin;
            layer.neurons[i].d2fdx2 = -tmpcos;
#ifdef HIGH_DERIVATIVES
            layer.neurons[i].d3fdx3 = tmpsin;
            layer.neurons[i].d4fdx4 = tmpcos;
#endif 
        }
...
#ifdef HIGH_DERIVATIVES
	    double e=1/dtmp-1;// exp(-x)
	    double e2=e*e;
	    double e3=e2*e;
	    double e4=e2*e2;
	    double dtmp2=dtmp*dtmp;
	    double dtmp4=dtmp2*dtmp2;
	    double dtmp5=dtmp4*dtmp;
            layer.neurons[i].d3fdx3 = -dtmp4*(e3-4*e2+e);
            layer.neurons[i].d4fdx4 = -dtmp5*(e4-11*e3+11*e2-e);
#endif 
        }
...
#ifdef HIGH_DERIVATIVES
            layer.neurons[i].d3fdx3 =-dtmp;
            layer.neurons[i].d4fdx4 = dtmp;
#endif 
        }
