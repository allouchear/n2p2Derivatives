...
void Structure::readFromLines(vector<string> const& lines)
{
         ...
         line != lines.end(); ++line)
    {
	    ...
	    for(int i=0;i<3;i++)
	    for(int j=0;j<3;j++)
               atoms.back().dDipoleRef[i][j]   = 0;

		/*
	    for(int i=0;i<3;i++)
	    for(int j=0;j<3;j++)
               atoms.back().dDipoleRef[i][j]   = atof(splitLine.at(i*3+j+10).c_str());
		*/
#endif
        ...
        else if (splitLine.at(0) == "dipole")
        {
            dipoleRef[0] = atof(splitLine.at(1).c_str());
            dipoleRef[1] = atof(splitLine.at(2).c_str());
            dipoleRef[2] = atof(splitLine.at(3).c_str());
        }
				...
				)
{
    ...
    dipole *= convDipole;
    dipoleRef *= convDipole;

    for (vector<Atom>::iterator it = atoms.begin(); it != atoms.end(); ++it)
    {
#ifdef DIPOLE_DERIVATIVES
        it->toNormalizedUnits(convEnergy, convLength,convDipole);
#else
                                ...
                                double convDipole)
{
    ...
    {
#ifdef DIPOLE_DERIVATIVES
        it->toPhysicalUnits(convEnergy, convLength, convDipole);
#else
...
void Structure::updateRmseCharge(double& rmse, size_t& count) const
{
    count++;
    rmse += (chargeRef - charge) * (chargeRef - charge)
          / (numAtoms * numAtoms);

    return;
}

void Structure::updateRmseDipole(double rmse[], size_t& count) const
{
    count++;
    for( int i=0; i<3;i++)
    	rmse[i] += (dipoleRef[i] - dipole[i]) * (dipoleRef[i] - dipole[i])
          / (numAtoms * numAtoms);

    return;
}

string Structure::getChargeLine() const
{
    return strpr("%10zu %16.8E %16.8E",
                 index,
                 chargeRef / numAtoms,
...
string Structure::getDipoleLine() const
{
    string  v = strpr("%10zu", index);
    for (int i =0; i<3;i++)
    {
        string  va = strpr(" %16.8E %16.8E",
                 dipoleRef[i] / numAtoms,
                 dipole[i] / numAtoms);
        v = v + va;
    }
    //v = v + "\n";
...
void Structure::writeToFile(ofstream* const& file, bool const ref) const
{
        ...
        if (ref)
        {
            (*file) << strpr("atom %24.16E %24.16E %24.16E %2s %24.16E %24.16E"
				...
				*/
                             ...
                             it->charge,
                             0.0,
                             it->fRef[0], it->fRef[1], it->fRef[2]
#ifdef DIPOLE_DERIVATIVES
				,
                             it->dDipoleRef[0][0], it->dDipoleRef[0][1], it->dDipoleRef[0][2],
                             it->dDipoleRef[1][0], it->dDipoleRef[1][1], it->dDipoleRef[1][2],
                             it->dDipoleRef[2][0], it->dDipoleRef[2][1], it->dDipoleRef[2][2]
#endif
				);
        }
        else
...
#ifdef DIPOLE_DERIVATIVES
				,
                             it->dDipole[0][0], it->dDipole[0][1], it->dDipole[0][2],
                             it->dDipole[1][0], it->dDipole[1][1], it->dDipole[1][2],
                             it->dDipole[2][0], it->dDipole[2][1], it->dDipole[2][2]
#endif
				);
    ...
    if (ref) (*file) << strpr("dipole %24.16E %24.16E %24.16E\n", dipoleRef[0], dipoleRef[1],dipoleRef[2]);
    else     (*file) << strpr("dipole %24.16E %24.16E %24.16E\n", dipole[0], dipole[1],dipole[2]);
