...
void   SymmetryFunctionAngularWide::computeDerivatives(Atom& atom, Derivatives& deriv) const
{
        	...
        	if ((e1 == nej || e2 == nej) && rij < rc)
        	{
        			...
        			double const rik = nk.d;
                		if ((e1 == nej && e2 == nek) || (e2 == nej && e1 == nek))
                		{
                    			if (rik < rc)
                    			{
						dum.computeG(zeta, lambda, eta, nj.dr.r, nk.dr.r, rs, fc);
...
void  SymmetryFunctionAngularWide::compute2Derivatives(Atom& atom, std::vector< std::vector<double> >& deriv) const
{
        	...
        	if ((e1 == nej || e2 == nej) && rij < rc)
        	{
        			...
        			double const rik = nk.d;
                		if ((e1 == nej && e2 == nek) || (e2 == nej && e1 == nek))
                		{
                    			if (rik < rc)
                    			{
						getDerivativesG(nj.dr.r, nk.dr.r, lambda, zeta, eta, rs, firstDeriv, secondDeriv, fc);
