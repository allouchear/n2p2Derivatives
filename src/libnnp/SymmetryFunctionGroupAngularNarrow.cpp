...
void  SymmetryFunctionGroupAngularNarrow::computeDerivatives(Atom& atom, Derivatives& deriv) const
{
	double rc2 = rc*rc;
        	...
        	if ((e1 == nej || e2 == nej) && rij < rc)
        	{
        			...
        			double const rik = nk.d;
						...
						if (rjk < rc2)
						{
    							for (size_t l = 0; l < members.size(); ++l)
							{
								dum.computeG(zeta[l], lambda[l], eta[l], nj.dr.r, nk.dr.r,  vrjk,  rs[l], fc);
								deriv.addDerives(dum, atom.index,  nj.index, nk.index , scalingFactors[l]);
							}
						}
                    			} // rik <= rc
                		} // elem
...
// deriv is a 3Nx3N matrix where N = # of atomes
// deriv matrix must be initialized before calling of compute2Derivatives method.
void  SymmetryFunctionGroupAngularNarrow::compute2Derivatives(Atom& atom, std::vector< std::vector<double> >& deriv) const
{
    	double const rc2    = rc * rc;
        	...
        	if ((e1 == nej || e2 == nej) && rij < rc)
        	{
        			...
        			double const rik = nk.d;
						...
						if (rjk < rc2)
						{
    							for (size_t l = 0; l < members.size(); ++l)
							{
								getDerivativesG(nj.dr.r, nk.dr.r, vrjk, lambda[l], zeta[l], eta[l], rs[l], firstDeriv, secondDeriv, fc);
								setSecondDerivesijk(secondDeriv, atom.index, nj.index, nk.index, scalingFactors[l], deriv);
							}
						}
                    			} // rik <= rc
                		} // elem
