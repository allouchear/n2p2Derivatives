...
void  SymmetryFunctionGroupAngularWide::computeDerivatives(Atom& atom, Derivatives& deriv) const
{
	...
	for (size_t j = 0; j < numNeighbors - 1; j++)
	{
        	...
        	if ((e1 == nej || e2 == nej) && rij < rc)
        	{
        			...
        			double const rik = nk.d;
						...
						for (size_t l = 0; l < members.size(); ++l)
						{
							dum.computeG(zeta[l], lambda[l], eta[l], nj.dr.r, nk.dr.r, rs[l], fc);
...
// deriv is a 3Nx3N matrix where N = # of atomes
// deriv matrix must be initialized before calling of compute2Derivatives method.
void  SymmetryFunctionGroupAngularWide::compute2Derivatives(Atom& atom, std::vector< std::vector<double> >& deriv) const
{
	...
	for (size_t j = 0; j < numNeighbors - 1; j++)
	{
        	...
        	if ((e1 == nej || e2 == nej) && rij < rc)
        	{
        			...
        			double const rik = nk.d;
						...
						for (size_t l = 0; l < members.size(); ++l)
						{
						getDerivativesG(nj.dr.r, nk.dr.r, lambda[l], zeta[l], eta[l], rs[l], firstDeriv, secondDeriv, fc);
