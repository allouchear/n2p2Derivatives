...
void  SymmetryFunctionGroupRadial::computeDerivatives(Atom& atom, Derivatives& deriv) const
{
    ...
    for (size_t j = 0; j < atom.numNeighbors; ++j)
    {
		...
		deriv.addDerives(dum, atom.index, n.index , scalingFactors[k]);
	    }
        }
    }
}
// deriv is a 3Nx3N matrix where N = # of atomes
// deriv matrix must be initialized before calling of compute2Derivatives method.
void  SymmetryFunctionGroupRadial::compute2Derivatives(Atom& atom, std::vector< std::vector<double> >& deriv) const
{
    ...
    for (size_t j = 0; j < atom.numNeighbors; ++j)
    {
		...
		setSecondDerivesij(secondDeriv, atom.index, n.index , scalingFactors[k], deriv);
	    }
        }
    }
}
