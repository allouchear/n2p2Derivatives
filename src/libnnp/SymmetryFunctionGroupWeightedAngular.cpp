...
void  SymmetryFunctionGroupWeightedAngular::computeDerivatives(Atom& atom, Derivatives& deriv) const
{
    	double const rc2    = rc * rc;
        			...
        			double const rik = nk.d;
						...
						if (rjk < rc2)
						{
    							for (size_t l = 0; l < members.size(); ++l)
							{
								...
								double scalf = elementMap.atomicNumber(nej)*elementMap.atomicNumber(nek)*scalingFactors[l];
...
// deriv is a 3Nx3N matrix where N = # of atomes
// deriv matrix must be initialized before calling of compute2Derivatives method.
void  SymmetryFunctionGroupWeightedAngular::compute2Derivatives(Atom& atom, std::vector< std::vector<double> >& deriv) const
{
    	double const rc2    = rc * rc;
