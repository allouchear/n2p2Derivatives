...
void  SymmetryFunctionGroupWeightedRadial::computeDerivatives(Atom& atom, Derivatives& deriv) const
{
    ...
    for (size_t j = 0; j < atom.numNeighbors; ++j)
    {
        ...
        if (n.d < rc)
        {
            for (size_t k = 0; k < members.size(); ++k)
	    {
		dum.computeG(lm, eta[k], n.dr.r, rs[k], fc);
		double scalf = scalingFactors[k]*elementMap.atomicNumber(n.element);
...
// deriv is a 3Nx3N matrix where N = # of atomes
// deriv matrix must be initialized before calling of compute2Derivatives method.
void  SymmetryFunctionGroupWeightedRadial::compute2Derivatives(Atom& atom, std::vector< std::vector<double> >& deriv) const
{
    ...
    for (size_t j = 0; j < atom.numNeighbors; ++j)
    {
        ...
        if (n.d < rc)
        {
            for (size_t k = 0; k < members.size(); ++k)
	    {
		getDerivativesG(n.dr.r, eta[k], rs[k], secondDeriv, fc);
		setSecondDerivesij(secondDeriv, atom.index, n.index , scalingFactors[k]*elementMap.atomicNumber(n.element), deriv);
	    }
        }
    }
}
