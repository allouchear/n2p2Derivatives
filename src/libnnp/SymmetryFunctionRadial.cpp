...
void  SymmetryFunctionRadial::computeDerivatives(Atom& atom, Derivatives& deriv) const
{
    //static int tmp = 0;
    ...
    for (size_t j = 0; j < atom.numNeighbors; ++j)
    {
		...
		deriv.addDerives(dum, atom.index, n.index , scalingFactor);
        }
    }
}
// deriv is a 3Nx3N matrix where N = # of atomes
// deriv matrix must be initialized before calling of compute2Derivatives method.
void  SymmetryFunctionRadial::compute2Derivatives(Atom& atom, std::vector< std::vector<double> >& deriv) const
{
    //static int tmp = 0;
    ...
    for (size_t j = 0; j < atom.numNeighbors; ++j)
    {
		...
		setSecondDerivesij(secondDeriv, atom.index, n.index , scalingFactor, deriv);
        }
    }
}
#endif
