...
// deriv matrix must be initialized before calling of computeDerivatives method.
// order is already defined in deriv.maxOrder
void  SymmetryFunctionWeightedRadial::computeDerivatives(Atom& atom, Derivatives& deriv) const
{
    ...
    for (size_t j = 0; j < atom.numNeighbors; ++j)
    {
        ...
        if (n.d < rc)
        {
		dum.computeG(lm, eta, n.dr.r, rs, fc);
		double scalf = scalingFactor*elementMap.atomicNumber(n.element);
		deriv.addDerives(dum, atom.index, n.index , scalf);
        }
    }
}
// deriv is a 3Nx3N matrix where N = # of atomes
// deriv matrix must be initialized before calling of compute2Derivatives method.
void  SymmetryFunctionWeightedRadial::compute2Derivatives(Atom& atom, std::vector< std::vector<double> >& deriv) const
{
    double secondDeriv[6][6];
    ...
    for (size_t j = 0; j < atom.numNeighbors; ++j)
    {
        ...
        if (n.d < rc)
        {
		getDerivativesG(n.dr.r, eta, rs, secondDeriv, fc);
		setSecondDerivesij(secondDeriv, atom.index, n.index , scalingFactor*elementMap.atomicNumber(n.element), deriv);
        }
    }
}
#endif
